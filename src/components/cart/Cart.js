import React, { useContext, useState } from "react";
import ProductContext from "../../store/ProductContext";
export const Cart = (props) => {
  const ProductCtx = useContext(ProductContext);
  let cartAllProducts = [...ProductCtx.carts.cartProducts];
  let cartProducts = cartAllProducts.filter(
    (cartitem) => cartitem.productName === props.productName
  );
  const incQuantity = () => {
    if (cartProducts[0].quantity >= 1) {
      cartProducts[0].quantity += 1;
      ProductCtx.carts.setCartProducts([...ProductCtx.carts.cartProducts]);
    }
  };
  const descQuantity = () => {
    if (cartProducts[0].quantity >= 2) {
      cartProducts[0].quantity -= 1;
      ProductCtx.carts.setCartProducts([...ProductCtx.carts.cartProducts]);
    }
  };
  const deleteCartHandler=()=>{
        let cartDelProducts = cartAllProducts.filter(
            (cartitem) => cartitem.productName != props.productName
        );
        ProductCtx.carts.setCartProducts(cartDelProducts);
  }

  return (
    <div className="container">
      <div className="product_cart">
        <div className="cartProductName">{props.productName}</div>
        <div className="cartQuantity">
          <button className="inc" onClick={incQuantity}>
            +
          </button>
          <div className="quantity">{cartProducts[0].quantity}</div>
          <button className="desc" onClick={descQuantity}>
            -
          </button>
          <img
            src="data:image/png;base64,"
            className="deleteCart"
            alt=""
            onClick={deleteCartHandler}
          />
        </div>
        <div className="clearfix1"></div>
        <div className="cartProductPrice">
          {"$" +
            cartProducts[0].quantity *
              Number(cartProducts[0].productPrice.substring(1))}
        </div>
      </div>
      <br/>
    </div>
  );
};
